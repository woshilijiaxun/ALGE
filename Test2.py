...
class ED_method:
    ...
    def compute_metrics(self):
        ...
        total_sum = np.sum([len(con[n]) + sum(
            [np.sum(nx.to_numpy_array(self.graphs[a], nodelist=self.node_list)[self.node_list.index(n)]) for a in
             range(self.L)]) for n in self.node_list])
...
def f_eigenvector_centrality(path, nodes_num, NetworkName):
    ...
    node_rank_f_eigenvector = [
        f_eigenvector_sorted_node.index(x) if x in f_eigenvector_sorted_node else len(f_eigenvector_sorted_node) for x
        in sir_list]
...
if __name__ == '__main__':
    ...
    for name in network:
        ...
        for t in [1.5]:
            ...
            nodes_ranking_dict[t] = {'dc': dc_dict, 'kshell': kshell_dict, 'glstm': glstm_dict, 'rcnn': rcnn_dict,
                                        'f-e': f_dict, 'prgc': prgc_dict,
    ...
    with open('[1.5]node_ranking_dict.pkl', 'wb') as f:
        pickle.dump(Result_sorted_dict, f)

    with open('[1.5]kendall[0.5-1.5].pkl', 'wb') as f:
        pickle.dump(Result_kendall, f)

    with open('[1.5]runtime.pkl', 'wb') as f:
